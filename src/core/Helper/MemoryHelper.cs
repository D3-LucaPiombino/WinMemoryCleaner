...
namespace WinMemoryCleaner.Core
{
    ...
    {
        private readonly LogHelper<MemoryHelper> LogHelper;

        public MemoryHelper(
            LogHelper<MemoryHelper> logger
        )
        {
            LogHelper = logger ?? throw new ArgumentNullException(nameof(logger));
        }
        /// <summary>
        /// Memory clean
        /// </summary>
        /// <param name="areas">Memory areas</param>
        public void Clean(Enums.Memory.Area areas)
        {
            ...
            if (areas.HasFlag(Enums.Memory.Area.ProcessesWorkingSet))
            {
                try
                {
                    ...
                    LogHelper.Info(
                        string.Format(CultureInfo.CurrentCulture, "{0} ({1})", Resources.MemoryHelperProcessesWorkingSet, Resources.LogCleaned.ToUpper(CultureInfo.CurrentCulture)));
                }
                catch (Exception e)
                {
                    LogHelper.Error(e, "Failed to clean memory");
                }
            }
            ...
            // Clean Standby List
            if (areas.HasFlag(Enums.Memory.Area.StandbyListLowPriority))
            {
                try
                {
                    CleanStandbyList(lowPriority: true);

                    LogHelper.Info(string.Format(CultureInfo.CurrentCulture, "{0} ({1})", Resources.MemoryHelperLowPriorityStandbyList, Resources.LogCleaned.ToUpper(CultureInfo.CurrentCulture)));
                }
                catch (Exception e)
                {
                    LogHelper.Error(e);
            ...
            if (areas.HasFlag(Enums.Memory.Area.StandbyList))
            {
                try
                {
                    CleanStandbyList(lowPriority: false);

                    LogHelper.Info(string.Format(CultureInfo.CurrentCulture, "{0} ({1})", Resources.MemoryHelperStandbyList, Resources.LogCleaned.ToUpper(CultureInfo.CurrentCulture)));
                }
                catch (Exception e)
                {
                    LogHelper.Error(e);
        ...
        private void CleanCombinedPageList()
        {
        ...
        /// </exception>
        private void CleanModifiedPageList()
        {
            // Windows minimum version
        ...
        private void CleanStandbyList(bool lowPriority = false)
        {
            // Windows minimum version
            if (!ComputerHelper.IsWindowsVistaOrAbove)
            {
        ...
        /// </exception>
        /// <exception cref="Win32Exception"></exception>
        private void CleanSystemWorkingSet()
        {
            // Windows minimum version
            if (!ComputerHelper.IsWindowsVistaOrAbove)
            {
        ...
        /// </summary>
        /// <exception cref="System.ComponentModel.Win32Exception"></exception>
        private void CleanProcessesWorkingSet()
        {
            // Windows minimum version
            if (!ComputerHelper.IsWindowsVistaOrAbove)
            {
